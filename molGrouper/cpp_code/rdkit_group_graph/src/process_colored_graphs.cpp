...
std::pair<std::vector<GroupGraph>, std::unordered_set<std::string>> process_nauty_graph_vcolg_output(
    ...
    for (const auto& port_combo : port_combos) {
        try {
            ...
            moleGraph = temp_gG.toMolecularGraph(nodeTypeToSmiles, nodeTypePortToIndex);
            std::string smiles = RDKit::MolToSmiles(*moleGraph);

            if (graph_basis.find(smiles) != graph_basis.end()) {
                continue;
            }
            graph_basis.insert(smiles);
    ...
    return {group_graphs_list, graph_basis};
}
